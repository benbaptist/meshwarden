...
export default defineComponent({
  ...
  template: `
    <div class="p-6 max-w-5xl mx-auto">
      <div v-if="!node" class="text-gray-500 text-sm">Loading…</div>
      <template v-else>
        <!-- Header -->
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div class="bg-gray-900 rounded-xl border border-gray-800 p-5">
            ...
            <div v-if="loadingStats" class="text-gray-500 text-sm">Loading…</div>
            <div v-else-if="!stats" class="text-gray-600 text-sm">Node offline or stats unavailable</div>
            <template v-else>
              <dl class="space-y-2 text-sm">
                <div class="flex justify-between">
                ...
                <div class="flex justify-between">
                  <dt class="text-gray-500">Noise Floor</dt>
                  <dd class="text-white">{{ fmt(stats.radio?.noise_floor, ' dBm') }}</dd>
                </div>
                <div class="flex justify-between">
                  <dt class="text-gray-500">Last RSSI / SNR</dt>
                ...
                <div class="flex justify-between">
                  <dt class="text-gray-500">Packets RX / TX</dt>
        ...
        <Modal :show="showRadioEdit" title="Edit Radio Config" @close="showRadioEdit = false">
          <form @submit.prevent="saveRadioConfig" class="space-y-4">
            <div>
              <label class="block text-sm text-gray-300 mb-1.5">Node Name</label>
              <input v-model="radioForm.name" type="text" class="w-full px-4 py-2.5 rounded-lg bg-gray-800 border border-gray-700 text-white text-sm focus:outline-none focus:border-mesh-500" />
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="block text-sm text-gray-300 mb-1.5">Frequency (MHz)</label>
                <input v-model.number="radioForm.freq" type="number" step="0.001" class="w-full px-4 py-2.5 rounded-lg bg-gray-800 border border-gray-700 text-white text-sm focus:outline-none focus:border-mesh-500" />
              </div>
              <div>
                <label class="block text-sm text-gray-300 mb-1.5">Bandwidth (kHz)</label>
                <input v-model.number="radioForm.bw" type="number" class="w-full px-4 py-2.5 rounded-lg bg-gray-800 border border-gray-700 text-white text-sm focus:outline-none focus:border-mesh-500" />
              </div>
              <div>
                <label class="block text-sm text-gray-300 mb-1.5">Spreading Factor</label>
                <input v-model.number="radioForm.sf" type="number" min="7" max="12" class="w-full px-4 py-2.5 rounded-lg bg-gray-800 border border-gray-700 text-white text-sm focus:outline-none focus:border-mesh-500" />
              </div>
              <div>
                <label class="block text-sm text-gray-300 mb-1.5">Coding Rate (5-8)</label>
                <input v-model.number="radioForm.cr" type="number" min="5" max="8" class="w-full px-4 py-2.5 rounded-lg bg-gray-800 border border-gray-700 text-white text-sm focus:outline-none focus:border-mesh-500" />
              </div>
            </div>
            <div>
